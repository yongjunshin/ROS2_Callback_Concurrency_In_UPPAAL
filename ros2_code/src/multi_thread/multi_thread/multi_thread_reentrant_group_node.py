...
class MultiThreadReentrantGroupNode(Node):

    def __init__(self):
        super().__init__('multi_thread_reentrant_group_node')
        ...
        self.timer_A = self.create_timer(1, self.publish_msg_A, callback_group=self.reentrant_group)
        self.timer_B = self.create_timer(1, self.publish_msg_B, callback_group=self.reentrant_group)
...
def main(args=None):
    ...
    node = MultiThreadReentrantGroupNode()
